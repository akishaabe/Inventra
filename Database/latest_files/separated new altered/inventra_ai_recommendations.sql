...
CREATE TABLE `ai_recommendations` (
  ...
  `reason` text,
  `generated_by` varchar(100) DEFAULT 'timefm',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `is_active` tinyint(1) DEFAULT '1',
  PRIMARY KEY (`id`),
  KEY `idx_ai_product_time` (`product_id`,`created_at`),
  CONSTRAINT `fk_airec_product` FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`) ON DELETE CASCADE ON UPDATE CASCADE
