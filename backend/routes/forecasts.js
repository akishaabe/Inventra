...
async function ensurePlaceholderProduct() {
  const [rows] = await composeDb.query(`SELECT product_id FROM products WHERE sku = ? LIMIT 1`, [FORECAST_PRODUCT_SKU]);
  ...
  await composeDb.query(`INSERT INTO inventory (product_id, quantity_available) VALUES (?, ?)`, [res.insertId, 0]);
  return res.insertId;
}
...
async function persistForecasts(productId, mlData) {
  ...
  const insertSql = `
    INSERT INTO forecasts
      (product_id, forecast_date, forecasted_demand, model_meta, created_at)
    VALUES ?
...
router.get("/", async (req, res) => {
  try {
    ...
    if (refresh) await composeDb.query(`DELETE FROM forecasts WHERE product_id = ?`, [placeholderProductId]);

    const mlResponse = await axios.post(ML_URL, { horizon: 14 });
